...
#define sd_init()	// spiMasterInit(SPICLKSLCAC,SSPENABLE|SSPMCLKD4);                    TRISC6=0;//SPI_Initialize()
#define sd_open()	// SPI_Open()
#define sd_tx(d)	SPI_TransReceive_FasFS(&hspi1,d)// spiPut(d)//SPI_Exchange8bit(d)
#define sd_rx()		SPI_TransReceive_FasFS(&hspi1,0xFF)// spiPut(0xFF)//SPI_Exchange8bit(0xFF)

uint8_t SPI_TransReceive_FasFS (SPI_HandleTypeDef *hspi,uint8_t val);
