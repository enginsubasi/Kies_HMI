...
)
{
	...
	if (i == _FS_LOCK)	/* The object is not opened */
		return (be || acc == 2) ? FR_OK : FR_TOO_MANmenu1LeftOffsetOPEN_FILES;	/* Is there a blank entry for new object? */
...
#if _FS_LOCK
					res = enq_lock() ? dir_register(&dj) : FR_TOO_MANmenu1LeftOffsetOPEN_FILES;
#else
					res = dir_register(&dj);
#endif
			...
			if (res == FR_OK) {
				...
				if (res == FR_OK) {
					if (dp->sclust) {
						...
						if (!dp->lockid)
							res = FR_TOO_MANmenu1LeftOffsetOPEN_FILES;
					} else {
